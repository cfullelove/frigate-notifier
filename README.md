...
A fail-closed MQTT service for custom review events. It caches a Home Assistant alarm state over WebSocket, applies ordered rules, asks Gemini to inspect a Frigate snapshot, and sends a Telegram photo followed by a Frigate video when the review ends.
...
Frigate snapshot and video endpoints use the first linked event; camera falls back to `cameras[0]`. Set `frigate.clip.source` to `clip` (the default) for the full recording, or `preview` to use Frigate's smaller time-range `preview.mp4`. Downloads are bounded to 50 MiB, retried only for transient HTTP failures, and videos are securely temporary files. Telegram captions are Unicode-safely limited to 1024 code points. State is in-memory only, so restarts intentionally lose correlation.
